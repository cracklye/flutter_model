...
class ModelEditViewStateChanged<T extends IModel>
    extends ModelEditViewStateEdit<T> {
  const ModelEditViewStateChanged([super.model]);
}

class ModelEditViewStateSaving<T extends IModel>
    extends ModelEditViewStateEdit<T> {
...
class ModelEditViewBloc2<T extends IModel>
  ...
  void _oModelEditViewEventSelectModel(ModelEditViewEventSelectModel<T> event,
      Emitter<ModelEditViewState<T>> emit) async {
    if (event.isEditMode) {
      emit(ModelEditViewStateEdit(event.model));
    } else {
      emit(ModelEditViewStateView(event.model));
    }
  }
